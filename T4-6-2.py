...
df = pd.read_csv('car.csv',encoding='ansi')
raw_X = df.values[:, :-1]
raw_y = df.values[:, -1]
X_names = np.array(df.columns[:-1])
X, X_v, y, y_v = train_test_split(raw_X, raw_y, test_size=0.2, random_state=462)
attributions = []
...
def print_tree(T, layer=0):
    """合理输出决策树"""
    if T.d == -1:
        print(' ' * 10 * layer, 'leaf (result : {})'.format(T.kind), sep='')
    else:
...
def decision_precision(X, y, T):
    ...
    for i in range(len(y)):
        res += int(decision_result(X[i], y[i], T))
    return res / len(y)
